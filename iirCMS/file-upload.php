...
// process file upload
  ...
  while(true) {
   	...
   	$oldaudiofile = "";
   	$oldaudiofile = $_POST['oldaudiofile'];
    ...
    if (DO_LOG) {
      ...
      if ($oldaudiofile != "") {
      	...
      	$sql = "update audio set log_ip = '$m_ip', name = '$name', description = '$desc', composer = '$composer', performer = '$performer', instrument = '$instrument', moreInfoURL = '$moreInfoURL' where log_id = '" . $oldaudiofile . "'";
      	//echo $sql . "<br/>";
      	$res = @mysql_query($sql);
      	if (!$res) {
	      ...
	      $m_ip = mysql_real_escape_string($_SERVER['REMOTE_ADDR']);
   	   	  $m_size = $filesize;
      	  $m_fname = mysql_real_escape_string($dest_filename);
      	  $sql = "insert into audio (log_filename,log_size,log_ip,name,description,composer,performer,instrument,moreInfoURL) values ('$m_fname','$m_size','$m_ip','$name','$desc','$composer','$performer','$instrument','$moreInfoURL')";
      	  //echo $sql . "<br/>";
      	  $res = @mysql_query($sql);
      	  if (!$res) {
			$errors[] = "Could not run query.";
